...
//import { FilterFileBrowserModel as dm_FilterFileBrowserModel} from '@jupyterlab/filebrowser';
 ...
 */
export class dm_DirListing extends Widget {
  /**
  ...
  constructor(options: dm_DirListing.IOptions) {
    super({
      node: (options.renderer || dm_DirListing.defaultRenderer).createNode()
    });
   ...
   */
  get model(): dm_FilterFileBrowserModel {
    return this._model;
  }
   ...
   */
  get renderer(): dm_DirListing.IRenderer {
   ...
   */
  get sortState(): dm_DirListing.ISortState {
   ...
   */
  get onItemOpened(): ISignal<dm_DirListing, Contents.IModel> {
   ...
   */
  sort(state: dm_DirListing.ISortState): void {
  ...
  setColumnVisibility(
    name: dm_DirListing.ToggleableColumn,
    visible: boolean
  ): void {
    if (visible) {
  ...
  protected handleOpen(item: Contents.IModel): void {
    ...
    if (item.type === 'directory') {
	  ...
	  console.log("handleOpen: ", localPath);
      this._model
        .cd(`/${localPath}`)
        .catch(error =>
          showErrorMessage(
  ...
  private _startDrag(index: number, clientX: number, clientY: number): void {
    ...
    for (const item of selectedItems) {
      ...
      } as dm_DirListing.IContentsThunk);
    }
  ...
  protected _model: dm_FilterFileBrowserModel;
  // signal to be consumed by other objects
  ...
  private _sortState: dm_DirListing.ISortState = {
    direction: 'ascending',
    key: 'name'
  };
  private _onItemOpened = new Signal<dm_DirListing, Contents.IModel>(this);
  ...
  private _renderer: dm_DirListing.IRenderer;
  private _searchPrefix: string = '';
  private _searchPrefixTimer = -1;
  private _inRename = false;
  private _isDirty = false;
  private _hiddenColumns = new Set<dm_DirListing.ToggleableColumn>();
}
...
export namespace dm_DirListing {
  ...
  export interface IRenderer {
    ...
    populateHeaderNode(
      node: HTMLElement,
      translator?: ITranslator,
      hiddenColumns?: Set<dm_DirListing.ToggleableColumn>
    ): void;
...
namespace Private {
  ...
  export function sort(
    ...
    state: dm_DirListing.ISortState
  ): Contents.IModel[] {
